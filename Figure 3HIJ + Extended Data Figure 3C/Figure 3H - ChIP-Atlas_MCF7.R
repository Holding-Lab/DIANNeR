...
df<-read.delim("ChIPAtlasData/NR3C1.Breast.tsv")
rowMetaData<-read.delim("ChIPAtlasData/ChIPAtlasRowMetaData.csv", sep=",")
...
#Heatmap column is hand removing any hits that are confounded with other conditions
#df<-df[df$Experiment %in% rowMetaData$SRX[rowMetaData$Heatmap == TRUE],]

# test<-data.frame(SRX<-df$Experiment[!df$Experiment %in%  rowMetaData$SRX],Antigen="ER",
#               Treatment1="No Estrogen",
#               Treament2="Dexamethasone",
#               Time="2 hours",
#               Antibody="Me",
#               Ref="",
#               Comments='',
...
#rowMetaData<-rbind(rowMetaData,test           )
